...
import { TaskapiMutationUseCases } from './taskapi-mutation-use-cases';
...
function createFirestoreHarness() {
  ...
  const firestore = {
    doc(path: string): MockDocument {
      ...
      return {
        id: segments[segments.length - 1] ?? 'doc',
        path,
      };
    },
...
describe('TaskapiMutationUseCases', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });

  it('records update and status-change history when task status changes', async () => {
    ...
    const service = new TaskapiMutationUseCases(harness.firestore as never);
    const result = await service.changeTaskStatus('user-1', {
      projectId: 'proj-1',
      taskId: 'task-1',
      status: 'done',
    });
  ...
  it('returns NOT_FOUND when deleting an unknown project', async () => {
    const harness = createFirestoreHarness();
    const service = new TaskapiMutationUseCases(harness.firestore as never);

    await expect(
      service.deleteProject('user-1', {
  ...
  it('keeps contract errors machine-readable when stored data is invalid', async () => {
    const harness = createFirestoreHarness();
    harness.reads.set('users/user-1/projects/proj-1', {
      id: 'proj-1',
      ownerUid: 'user-1',
    ...
    const service = new TaskapiMutationUseCases(harness.firestore as never);
    const result = await service.updateProject('user-1', {
      projectId: 'proj-1',
      name: 'Refined project',
      description: '',
      archived: false,
    });
